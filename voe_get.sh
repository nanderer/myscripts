...
if grep 'track kind="captions" label="' /tmp/veo_get.out; then 
echo subs=true;
...
#	lang=$(echo $i | cut -d '"' -f 5)
 #       echo lang=$lang
	suburl=https://voe.sx/$( grep 'track kind="captions" label="' /tmp/veo_get.out | cut -d '"' -f 8|head -n 1)	
lang=$(grep 'track kind="captions" label="' /tmp/veo_get.out | cut -d '"' -f 6| head -n 1)
echo suburl=$suburl
echo lang=$lang
